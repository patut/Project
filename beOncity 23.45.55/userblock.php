<div id="mainblock">
	<div id="userheader">
		<div id="image">
			<div id="imgcover"><img src="<?=$user['img']?>"/></div>
		</div>
		<div id="info">
			<div id="topline">
				<div id="usrnameblock"><?=$user['name']?></div>
                <? if($user['id'] == $_SESSION['id']) echo '<div id="settings"><a href="/settings">Настройки</a></div>'; ?>
			</div>
			<? 
			if (!empty($user['location'])) echo '<div id="location">'.$user['location'].'</div>';
            else if($user['id'] == $_SESSION['id']) echo '<div id="location"><a href="/settings">Добавить местоположение</a></div>';
			if (!empty($user['about'])) echo '<div id="about">'.$user['about'].'</div>'; 
            else if($user['id'] == $_SESSION['id']) echo '<div id="about"><a href="/settings">Добавить описание</a></div>';
			?>
		</div>
		<div class="clearfix"></div>
	</div>
	<div id="userwall">
		<div id="walltop">Последние события от пользователя <?=$user['name']?></div>
		<div id="blockfall">
            ...
            $id = $user['id'];
            $count = 0;
            $q = mysql_query("SELECT * FROM events WHERE author_id='$id' ORDER BY timestamp DESC", $db);
            while ($event = mysql_fetch_array($q)) {
                ...
                $addpost .= '<div class="post" id="post'.$event['id'].'">';
                    ...
                    $addpost .= '</div>';
                    $addpost .= '<div class="del fl_r">';
                        if($_SESSION['id']==$user['id']) $addpost .= '<img onclick="del(\''.$event['id'].'\')" src="/img/del.png"/>';
                    $addpost .= '</div>';
                    $addpost .= '<div class="clearfix"></div>';
                $addpost .= '</div>';
                //------------//
            ...
            if ($count==0) echo '<div id="noevents">Событий пока еще нет.'; 
            if(($user['id'] == $_SESSION['id']) && ($count==0)) echo '<a href="/update">Создать событие?</a></div>';
            else echo '</div>';
        ?>
    ...
    <div id="sidebar">
        <div class="block">
            <?
            $countEvents = 0;
            $q = mysql_query("SELECT * FROM willgo WHERE userid='$id' ORDER BY id DESC", $db);
            $addpost = '';
        ...
        <div class="block">
            <?
            $countEvents = 0;
            $q = mysql_query("SELECT * FROM beento WHERE userid='$id' ORDER BY id DESC", $db);
            $addpost = '';
            while ($eventId = mysql_fetch_array($q)) {                
